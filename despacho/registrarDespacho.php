                ...
                $fila=mysqli_fetch_array($consultaq);
                    if($devObjeto[$i]==1){
                        if($cantidad[$i]!=$fila['devolucion']){
                            $query = "UPDATE sop__despacho_db SET devolucion=IF(cantidad<$cantidad[$i]+1,cantidad,$cantidad[$i]) WHERE subtipo='$subtipo' and id_material='".$codigo[$i]."' and id_evento_acc=$idEvento;";
                            $consulta=mysqli_query($conexion,$query);    
                        }
                    }else{
